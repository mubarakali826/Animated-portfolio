...
/*--------------------------------------------------
Function WMG Lottie Initialization (Global)
---------------------------------------------------*/
	...
	window.initWMGLottie = function() {
		...
		function loadLottieLibrary(callback) {
			...
			script.onload = function() {
				console.log('✅ [WMG] Lottie library loaded successfully');
				callback();
			};
			script.onerror = function() {
				console.error('❌ [WMG] Failed to load Lottie library');
			};
		...
		function initAnimations() {
			...
			// Animation paths with correct directory structure
			const animationConfigs = [
				{ container: 'lottie1', path: 'assets/lottie/activities.json' },
				{ container: 'lottie2', path: 'assets/lottie/insights.json' },
				{ container: 'lottie3', path: 'assets/lottie/affinity-mapping.json' },
				{ container: 'lottie4', path: 'assets/lottie/discovery.json' }
			];
			...
			console.log('🎬 Initializing Lottie animations...', 'lottie available:', typeof lottie);
				...
				console.log(`Loading animation ${index + 1}: ${config.path}`);
				...
				try {
					...
					animation.addEventListener('loopComplete', function() {
						console.log(`Animation ${index + 1} loop completed`);
					});
					...
					animation.addEventListener('DOMLoaded', function() {
						console.log(`✅ [LOTTIE] Animation ${index + 1} loaded successfully!`);
					});
					
				} catch (error) {
					console.error(`Error loading animation ${index + 1}:`, error);
					container.innerHTML = `<div style="display: flex; align-items: center; justify-content: center; height: 100%; color: #999;">Animation ${index + 1} failed to load</div>`;
				}
			});

			// Wait a moment for animations to load, then set up scroll triggers
			setTimeout(() => {
				console.log('Setting up scroll triggers...');
				
				if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
					// Set up scroll-triggered animations for each section
					document.querySelectorAll('.lottie-section').forEach((section, index) => {
						ScrollTrigger.create({
							trigger: section,
							start: "top 70%",
							onEnter: () => {
								if (lottieAnimations[index]) {
									console.log(`▶️ Playing animation ${index + 1}`);
							...
							onEnterBack: () => {
								if (lottieAnimations[index]) {
									console.log(`🔄 Resetting animation ${index + 1}`);
									lottieAnimations[index].stop();
									lottieAnimations[index].play();
								}
							}
					...
					console.log('GSAP/ScrollTrigger not available, playing first animation');
					// Fallback: Just play the first animation
					if (lottieAnimations[0]) {
						lottieAnimations[0].play();
					}
				}
			}, 1000); // Give animations 1 second to load

			// Add scroll prompt fade effect
			...
			// GSAP ScrollTrigger for next-hero-title color change
			if (typeof gsap !== 'undefined' && typeof ScrollTrigger !== 'undefined') {
				var nextCase = document.querySelector('.next-hero-title');
				var imageArea = document.querySelector('.next-project-image');
				if (nextCase && imageArea) {
					ScrollTrigger.create({
						trigger: imageArea,
						start: 'top center',
						end: 'bottom center',
						onEnter: function() { nextCase.classList.add('is-over-image'); },
						onEnterBack: function() { nextCase.classList.add('is-over-image'); },
						onLeave: function() { nextCase.classList.remove('is-over-image'); },
						onLeaveBack: function() { nextCase.classList.remove('is-over-image'); }
					});
				}
			}
		}
...
---------------------------------------------------*/	
	...
	window.LoadViaAjax = function() {	
		...
		// Initialize WMG Lottie animations if on Warner Music Group page
		if (window.location.pathname.includes('warner-music-group') || document.querySelector('#lottie1')) {
			if (typeof window.initWMGLottie === 'function') {
				console.log('🔥 [AJAX] Calling initWMGLottie via AJAX');
				window.initWMGLottie();
			} else {
